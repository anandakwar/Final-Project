...
def same_day(num,w_day):
    ...
    for val in s_days:
        ...
        if (prep_date_format.day_name() == w_day) and (x<num):
            ...
            val=0
    if len(cp_list)==0:
        print("No occurences were found, exiting...")
    elif len(cp_list)< num :
        print("Not enough occurences were found, plotting",end=" ")
        print(x,end=" occurences\n")
    return cp_list
...
def findfile(DATA_FILES_DIR):
    filenames = []
    s3 = boto3.client('s3')
    choice = int(input("choose an option : 1.dates               2.different days in the week           3.same day in the week\n"))
    if choice==2 :
        ...
        w_day = str(input("Please enter the day :\n"))
        num = int(input("Please enter number of occurences :\n"))
        filenames = same_day(num,w_day)
    else :
...
if __name__ == '__main__':
    ...
    num = findfile(DATA_FILES_DIR)   # on your computer open an empty folder called downloads and put here the path
    if num ==0:
        quit(0)
